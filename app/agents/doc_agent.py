...
class DocAgent(BaseArtifactAgent[str]):
    _agent_name = "DocAgent"
...
def build_doc_artifact(task: AgentPilotTask) -> str:
    return _doc_agent.build(task)


def build_fallback_doc(task: AgentPilotTask) -> str:
    return _doc_agent._build_fallback(task, _doc_agent._brief(task))
